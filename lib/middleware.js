...
function blankOutNonMembers(config, curSet, log, request, response) {
    if (curSet) {
        const url = request.url.split('?')[0];
        const requestedFile = curSet.find(u => {
            return url.indexOf(u.replace(config.base, '')) > -1;
        });
        if (!requestedFile) {
...
function notInSet(config, log, request, response) {
    ...
    if (id) {
        ...
        return blankOutNonMembers(config, curSet, log, request, response);
    }
}
